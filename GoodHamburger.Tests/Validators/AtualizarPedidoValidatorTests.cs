...
{
    public class AtualizarPedidoValidatorTests
    {
        private readonly AtualizarPedidoValidator _validator;

        public AtualizarPedidoValidatorTests()
        {
            _validator = new AtualizarPedidoValidator();
        }

        [Fact]
        public async Task Validator_PedidoValido_NaoDeveRetornarErros()
        {
            var command = PedidoBuilder.BuildAtualizarPedidoCommand(1, TipoItem.Sanduiche, TipoItem.Batata);
        ...
        {
            var command = new AtualizarPedidoCommand { Id = 1, Itens = [] };
